...
def eval_rouge_n(y_candidate, y_reference, n = 2):
  ...
  for i in range(m):
    ...
    rouge_score.append(num_match/len(list(ngram_ref)))
...
def generate_ngrams(input_list, n):
  ...
  for i in range(len(gram_list)-1):
    gram_list.append(input_list[i]+input_list[i+1])
  return gram_list
...
def eval_rouge_l(y_candidate, y_reference):
  ...
  if(recall + beta * beta * precision) != 0:
    ...
    f1 = (1 + beta * beta) * precision * recall/0.001
  return f1
...
def test(y_candidate, y_reference):
  # strA = "ABCBDAB"
  # strB = "BDCABA"
  # m = LCS(strA, strB)
